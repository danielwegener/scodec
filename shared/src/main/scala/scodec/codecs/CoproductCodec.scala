...
final class CoproductCodecBuilder[C <: Coproduct, L <: HList, R] private[scodec] (
  codecs: L, cToR: C => Attempt[R], rToC: R => Attempt[C]
)(implicit aux: ToCoproductCodecs[C, L]) {
...
object CoproductCodecBuilder {
  ...
  implicit def transformInstance[C <: Coproduct, L <: HList]: Transform[({type l[a] = CoproductCodecBuilder[C, L, a]})#l] = new Transform[({type l[a] = CoproductCodecBuilder[C, L, a]})#l] {
    def exmap[A, B](builder: ({type l[a] = CoproductCodecBuilder[C, L, a]})#l[A], f: A => Attempt[B], g: B => Attempt[A]) =
      builder.exmap(f, g)
  }

  implicit def toTransformSyntax[C <: Coproduct, L <: HList, A](ccb: CoproductCodecBuilder[C, L, A]): TransformSyntax[({type l[a] = CoproductCodecBuilder[C, L, a]})#l, A] =
